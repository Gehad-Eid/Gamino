...
if (!isset($_POST["update"])) {
    ...
    $id=$_GET['id'];
        ...
        if(empty($_FILES["photo"]["name"])){
            $targetFilePath = $_POST["pic"];
            $vidTargetFilePath = $_POST["vid"];
            $pic=$targetFilePath;
            $vid=$vidTargetFilePath;
        }

        $message=str_replace("'","''",$message);//replace "" and '' 
        $date=date("d F Y");
        $GameURL=trim($_POST["GameStory"]);
        $GameURL = str_replace("/watch?v=","/embed/",$GameURL);

    
        if(!empty($_FILES["photo"]["name"]) || $pic !==''){
            // Allow certain file formats
            $allowTypes = array('jpg','png','jpeg');
            if(in_array($fileType, $allowTypes) || $pic){
                // Upload file to server
                if(move_uploaded_file($_FILES["photo"]["tmp_name"], $targetFilePath) || $pic){
